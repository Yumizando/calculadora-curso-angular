...
export class CalculadoraComponent implements OnInit {
  ...
  /**
   * Adiciona o número selecionado para fazer o cálculo posteriormente.
   * 
   * @param numero string
   * @return void
   */
  adicionarNumero(numero: string): void{
    if(this.operacao === null){
      ...
      this.numero1 = this.concatenarNumero(this.numero2, numero);
    }
  }
  ...
  concatenarNumero(numAtual: string, numConcat: string): string{
    ...
    // se o primeiro digito for '.', concatena '0' antes do ponto
    if(numConcat === '.' && numAtual === ''){
      return '0.'
    }

    //caso '.' for digitado e ja cotenha um '.', apenas retorna o numAtual
   ...
   * Caso já possua uma operação selecionada, 
   * executa a operação anterior e define a nova operação.
   * 
